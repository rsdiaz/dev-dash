...
const fetcher = async () =>
  await fetch('http://localhost:4000/trends', { method: 'GET' }).then(async (response) =>
    await response.json()
  )
    ...
    >
      <div className='flex justify-start items-center gap-3'>
        ...
        <h2 className='font-medium text-sm'>
          <a href={trend.url} rel='nofollow noreferrer' target='_blank'>
            {trend.name}
          </a>
          <br />
...
function Trends () {
  ...
  return trends?.map((trend: any) => <Trend key={trend.id} trend={trend} />)
}
