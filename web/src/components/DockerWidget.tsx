...
const fetcher = () =>
  fetch("http://localhost:4000/containers", { method: "GET" }).then((response) =>
    response.json()
  );
    ...
    >
      <div className="flex justify-start items-center gap-3">
        <figure className="relative w-12 h-12 flex-none">
          ...
          <span className={`absolute right-0 bottom-0 inline-block w-3.5 h-3.5 rounded-full ${containers.status === 200 ? 'bg-green-500' : 'bg-red-500'} border-2 border-white dark:border-gray-950`}></span>
        </figure>
        <h2 className="font-medium text-sm">
          Docker
          <br />
          <span className="text-gray-500 text-xs">{containers.status === 200 ? 'running' : 'not running 🚨'}</span>
        </h2>
      </div>
      <div className="flex justify-between items-center gap-2">
        <section className="flex flex-1 items-center gap-2 py-1 px-2 rounded-md dark:text-white">
          ...
          <span>{containers.containers ? containers.containers : '0'}</span>
        </section>
        <section className="flex flex-1 items-center gap-2 py-1 px-2 rounded-md dark:text-white">
          <svg
            width="18"
          ...
          <span>{containers.paused ? containers.paused : '0'}</span>
        </section>
        <section className="flex flex-1 items-center gap-2 py-1 px-2 rounded-md dark:text-white">
          <svg
            width="18"
          ...
          <span>{containers.running ? containers.running : '0'}</span>
        </section>
        <section className="flex flex-1 items-center gap-2 py-1 px-2 rounded-md dark:text-white">
          <svg
            width="18"
          ...
          <span>{containers.stopped ? containers.stopped : '0'}</span>
        </section>
      </div>
    </article>
  );
